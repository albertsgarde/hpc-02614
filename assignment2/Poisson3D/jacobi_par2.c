...
double 
jacobi_inner_par2(double ***u, double ***old_u, double ***f, const int N) {
...
int jacobi_par2(double *** u, double *** old_u, double ***f, const int N, const int iter_max, const double threshold) {
    int iter = 0;
    ...
    while (delta_norm > threshold && iter < iter_max) {
        ...
        #pragma omp barrier
        double _delta = jacobi_inner_par2(u, old_u, f, N);
