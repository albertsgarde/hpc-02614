...
double
gauss_seidel_inner_par1(double ***u, double ***f, const int N) {
...
int
gauss_seidel_par1(double ***u, double ***f, const int N, const int iter_max, const double threshold) {
    int iter = 0;
    double delta_norm = INFINITY;

    while (delta_norm > threshold && iter < iter_max) {
        delta_norm = gauss_seidel_inner_par1(u, f, N);
        ++iter;
    }
