...
double 
jacobi_inner_par1(double ***u, double ***old_u, double ***f, const int N) {
...
int jacobi_par1(double *** u, double *** old_u, double ***f, const int N, const int iter_max, const double threshold) {
    int iter = 0;
    ...
    while (delta_norm > threshold && iter < iter_max) {
        ...
        u = old_u;
        old_u = tmp;
        delta_norm = jacobi_inner_par1(u, old_u, f, N);
        ++iter;
    }
    return iter;
}