...
void jacobi_inner_off_1(double ***u, double ***old_u, double ***f, const int N) {

    const double one_sixth = 1./6.;
    const double grid_spacing_sq = grid_spacing(N)*grid_spacing(N);
    #pragma omp target data map(to: old_u[0:(N+2)][0:(N+2)][0:(N+2)], f[0:(N+2)][0:(N+2)][0:(N+2)]) map(from: u[0:(N+2)][0:(N+2)][0:(N+2)])
    {
...
int jacobi_off_1(double *** u, double *** old_u, double ***f, const int N, const int iter_max, const double threshold, const bool frobenius) {
    int iter = 0;
    double delta_norm = INFINITY;
       ...
       while (delta_norm > threshold && iter < iter_max) {
        ...
        u = old_u;
        old_u = tmp;
        jacobi_inner_off_1(u, old_u, f, N);
        /*if (frobenius) {
