...
void jacobi_inner_gpu_async(double ***d_u_0, double ***d_old_u_0, double ***d_f_0, double ***d_u_1, double ***d_old_u_1, double ***d_f_1, const int N) {
    ...
    #pragma omp target nowait teams loop is_device_ptr(d_u_0, d_old_u_0, d_f_0)\
        collapse(3) num_teams(n_half-1) thread_limit(128) 
    ...
    #pragma omp target nowait teams loop is_device_ptr(d_u_0, d_old_u_0, d_f_0,  d_u_1, d_old_u_1, d_f_1) device(1) \
        collapse(2) num_teams(1) thread_limit(128) 
    for (int j=1; j < (N + 1); j++){
        for (int k=1; k < (N + 1); k++){
            //printf("Mid: (%d,%d,%d) \n", n_half, j,k);
    ...
    #pragma omp target nowait teams loop is_device_ptr(d_u_1, d_old_u_1, d_f_1) \
        collapse(3) num_teams(n_half) thread_limit(128) 
    for (int i = 1; i < n_half-1; i++){
        for (int j=1; j < (N + 1); j++){
            for (int k=1; k < (N + 1); k++){
                //printf("Loop2 iteration: (%d,%d,%d) \n", i+n_half,j,k);
...
int jacobi_gpu_async(double *** u, double *** old_u, double ***f, const int N, const int iter_max, const double threshold, const bool frobenius) {
    ...
    omp_target_memcpy(d_f_data_0, u[0][0], (grid_size/2 - 1) * grid_size * grid_size * sizeof(double),
    ...
    omp_target_memcpy(d_f_data_1, u[1*N/2 - 1][0], (grid_size/2 + 1) * grid_size * grid_size * sizeof(double),
            0, 0, omp_get_default_device(), omp_get_initial_device());
   
    omp_set_default_device(0);
    omp_target_memcpy(d_u_data_0, u[0][0], (grid_size/2 - 1) * grid_size * grid_size * sizeof(double),
            0, 0, omp_get_default_device(), omp_get_initial_device());
            
    omp_set_default_device(1);
    omp_target_memcpy(d_u_data_1,u[1*N/2 - 1][0],  (grid_size/2 + 1) * grid_size * grid_size * sizeof(double),
            0, 0, omp_get_default_device(), omp_get_initial_device());
    ...
    while (delta_norm > threshold && iter < iter_max) {
        ...
        omp_set_default_device(0);
        omp_target_memcpy(u[0][0], d_u_data_0, (grid_size/2 - 1) * grid_size * grid_size * sizeof(double),
            0, 0, omp_get_initial_device(), omp_get_default_device());

        omp_set_default_device(1);
        omp_target_memcpy(u[(N/2) - 1][0], d_u_data_1, (grid_size/2 + 1) * grid_size * grid_size * sizeof(double),
            0, 0, omp_get_initial_device(), omp_get_default_device());
